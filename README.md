...
To avoid data duplication on GitHub, inside the ```/example-outcome/data/``` we do not store again the raw data folders (```from_sensors/0_raw``` and ```from_uppaal/0_raw```), since they are already available in the ```/data/``` folder.
